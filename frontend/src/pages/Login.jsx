          ...
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div>
              ...
              <div style={{ position:'relative' }}>
                <input type={show?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" required style={{...inputStyle, paddingRight:40}}
                  onFocus={e=>e.target.style.borderColor='var(--accent)'} onBlur={e=>e.target.style.borderColor='var(--border)'}/>
            ...
            <button type="submit" disabled={loading}
              style={{ padding:'12px', borderRadius:10, fontSize:14, fontWeight:600, color:'#fff', background:'var(--accent)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8, opacity:loading?.6:1, transition:'opacity .2s' }}>
              {loading ? <><Loader2 size={16} className="animate-spin"/> Signing in...</> : 'Sign in'}
            </button>
          </form>

          <p style={{ textAlign:'center', marginTop:18, fontSize:13, color:'var(--text-2)' }}>
            No account? <Link to="/register" style={{ fontWeight:600, color:'var(--accent-2)' }}>Sign up</Link>
          </p>
        </div>
      </div>
    </div>
